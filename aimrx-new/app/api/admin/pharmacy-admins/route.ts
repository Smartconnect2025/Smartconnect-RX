...
import { welcomeEmailHtml } from "@core/services/email/emailTemplates";
    ...
    try {
      ...
      if (sendGridApiKey) {
        ...
        const emailHtml = welcomeEmailHtml({
          greeting: `Hello${full_name ? ` ${full_name}` : ""},`,
          message: `Your pharmacy administrator account has been successfully created for <strong>${pharmacyName}</strong>! You can now access SmartConnect RX to manage pharmacy operations, prescriptions, and settings.`,
          email,
