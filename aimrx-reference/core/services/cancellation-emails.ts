...
export function renderCancelRefund(
  ...
  if (type === "cancelled" && role === "patient") {
    const subject = `Your AIMRx order for ${data.medication} has been cancelled`;
    const body = `
      <p style="margin:0 0 20px;font-size:16px;line-height:26px;color:#334155;">Hi ${e(data.patientFirstName)},</p>
      <p style="margin:0 0 20px;font-size:16px;line-height:26px;color:#334155;">Your AIMRx prescription order for <strong>${e(data.medication)}</strong> has been cancelled.</p>
      ...
      <p style="margin:0;font-size:14px;line-height:22px;color:#64748b;">— AIM RX Portal</p>
    `;
    const text = `Hi ${data.patientFirstName},

Your AIMRx prescription order for ${data.medication} has been cancelled.
Reason: ${data.reason}.
...
— AIM RX Portal`;
    return {
      subject,
      html: brandedWrapper("Your Order Has Been Cancelled", headerColor, body),
      text,
    };
  }
  ...
  if (type === "cancelled" && role === "provider") {
    ...
    const body = `
      <p style="margin:0 0 16px;font-size:16px;line-height:26px;color:#334155;">Order <strong>#${e(data.orderNumber)}</strong> for <strong>${e(data.patientFullName)}</strong> (${e(data.medication)}, ${e(data.dose)}, qty ${e(data.quantity)}) was cancelled by AIMRx admin on ${e(data.date)}.</p>
      ...
      <p style="margin:0;font-size:14px;line-height:22px;color:#64748b;">— AIM RX Portal</p>
    `;
    const text = `Order #${data.orderNumber} for ${data.patientFullName} (${data.medication}, ${data.dose}, qty ${data.quantity}) was cancelled by AIMRx admin on ${data.date}.
Reason: ${data.reason}.
...
— AIM RX Portal`;
    return {
      subject,
      html: brandedWrapper("Prescription Order Cancelled", headerColor, body),
      text,
    };
  }

  if (type === "cancelled" && role === "admin") {
    const subject = `[AIMRx audit] CANCEL \u2014 order ${data.orderNumber} (${data.patientFullName})`;
...
system_logs id: ${data.systemLogId || "(pending)"}`;
  ...
  if (type === "refunded" && role === "patient") {
    const subject = `Refund issued for your AIMRx order \u2014 ${data.medication}`;
    const body = `
      <p style="margin:0 0 20px;font-size:16px;line-height:26px;color:#334155;">Hi ${e(data.patientFirstName)},</p>
      <p style="margin:0 0 20px;font-size:16px;line-height:26px;color:#334155;">A refund of <strong>$${e(data.refundAmount)}</strong> has been issued for your AIMRx order of <strong>${e(data.medication)}</strong>.</p>
...
A refund of $${data.refundAmount} has been issued for your AIMRx order of ${data.medication}. The refund returns to the original payment method on file. Please allow 3\u20135 business days for it to appear.
...
— AIM RX Portal`;
    return {
      subject,
      html: brandedWrapper("Refund Issued", headerColor, body),
      text,
    };
  }
  ...
  if (type === "refunded" && role === "provider") {
    ...
    const body = `
      <p style="margin:0 0 16px;font-size:16px;line-height:26px;color:#334155;">Order <strong>#${e(data.orderNumber)}</strong> for <strong>${e(data.patientFullName)}</strong> was cancelled and <strong>$${e(data.refundAmount)}</strong> was refunded by AIMRx on ${e(data.date)}.</p>
      ...
      <p style="margin:0;font-size:14px;line-height:22px;color:#64748b;">— AIM RX Portal</p>
    `;
    const text = `Order #${data.orderNumber} for ${data.patientFullName} was cancelled and $${data.refundAmount} was refunded by AIMRx on ${data.date}.
...
— AIM RX Portal`;
    return {
      subject,
      html: brandedWrapper("Order Refunded", headerColor, body),
      text,
    };
  }

  if (type === "refunded" && role === "admin") {
    const subject = `[AIMRx audit] REFUND $${data.refundAmount} \u2014 order ${data.orderNumber}`;
...
system_logs id: ${data.systemLogId || "(pending)"}`;
  ...
  return { subject: "AIMRx notification", html: "", text: "" };
}
...
export async function sendCancellationEmails(
  ...
  if (!SENDGRID_API_KEY) {
    return {
      sent,
      failed: [{ to: "(no key)", error: "SendGrid not configured" }],
    };
  }
  ...
  const data: CancelRefundData = {
    ...
    patientEmail,
    providerName: providerName || "your AIMRx prescriber",
