...
export async function getPrescriptionPdfBase64(
  ...
  try {
    ...
    // Convert blob to base64 via Node Buffer (non-blocking). Do not use a per-byte btoa loop — it blocks the event loop on large PDFs.
    const arrayBuffer = await data.arrayBuffer();
    const convertStart = Date.now();
    const base64 = Buffer.from(arrayBuffer).toString("base64");
    const convertMs = Date.now() - convertStart;
    console.log(
      `📄 [Storage] PDF base64 convert ok bytes=${arrayBuffer.byteLength} ms=${convertMs}`,
    );
