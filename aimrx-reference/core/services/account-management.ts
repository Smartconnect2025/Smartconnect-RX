  ...
  try {
    ...
    if (params.role === "provider") {
      ...
      if (params.companyName) {
        providerData.company_name = params.companyName;
      }
      ...
      if (params.companyName && providerRecord) {
        try {
          const { error: syncError } = await supabase.rpc("sync_provider_to_group_patients", {
            p_provider_id: providerRecord.id,
          });
          if (syncError) {
            console.error("Company patient sync RPC error (non-fatal):", syncError.message);
          }
        } catch (syncError) {
