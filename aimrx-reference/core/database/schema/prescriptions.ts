...
export const prescriptions = pgTable(
  "prescriptions",
  {
    ...
    order_group_id: uuid("order_group_id"), // Groups prescriptions submitted together in one cart
    ...
    // Provider Assistance audit field — set when an assistant (delegate)
    // submitted on behalf of the prescriber. NULL when the provider
    // submitted directly. ON DELETE SET NULL so the prescription row
    // survives deletion of the delegation while not pointing at an
    // orphaned UUID. Declared with a thunk reference; delegations.ts does
    // not import from this file so there is no circular-import risk.
    submitted_by_delegation_id: uuid("submitted_by_delegation_id").references(
      () => delegations.id,
      { onDelete: "set null" },
    ),

    // Pay-on-Terms reconciliation: timestamp marking when the admin settled
    // (collected payment for) this prescription. NULL = still owed.
    pay_on_terms_settled_at: timestamp("pay_on_terms_settled_at", {
      withTimezone: true,
    }),
  },
  (table) => [
