...
export const GroupsManagement: React.FC = () => {
  ...
  const handleAssignProvider = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`${provider?.prefix || "Dr."} ${provider?.first_name} ${provider?.last_name} assigned to group`);
                  ...
                  <SelectContent>
                    {unassignedProviders.map((provider) => (
                      <SelectItem key={provider.id} value={provider.id}>
                        {provider.prefix || "Dr."} {provider.first_name} {provider.last_name} — {provider.email}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
