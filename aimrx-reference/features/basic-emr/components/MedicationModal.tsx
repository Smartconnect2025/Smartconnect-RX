...
export function MedicationModal({
  ...
  const handleSaveMedication = async () => {
    ...
    try {
      ...
      const isEdit = isEditing && medication;
      const res = await fetch("/api/basic-emr/medications", {
        method: isEdit ? "PATCH" : "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(
          isEdit ? { id: medication!.id, ...medicationData } : medicationData,
        ),
      });
      const result = await res
        .json()
        .catch(() => ({ success: false, error: "Network error" })) as {
        success: boolean;
        error?: string;
      };
