...
export function PatientForm({ patient, isEditing = false }: PatientFormProps) {
  ...
  // Default the "Billing same as shipping" checkbox based on the saved patient
  // data. For NEW patients (no patient prop), default to true (most common
  // case). For EXISTING patients, only default to true when the patient has
  // no separate billing address on file OR the billing address already
  // matches the shipping (physical) address. Otherwise default to false so
  // the saved separate billing address is preserved and not auto-clobbered.
  const [billingSameAsAddress, setBillingSameAsAddress] = useState(() => {
    if (!patient) return true;
    const billing = patient.billing_address;
    const shipping = patient.physical_address || patient.address;
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      Critical safety information shown on the patient chart
                    </p>
                  </div>
                </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea
                          placeholder="e.g. Penicillin (rash, hives), Sulfa drugs (anaphylaxis), Peanuts. Leave blank if no known allergies."
                          className="w-full border-gray-300 rounded-lg min-h-[96px] resize-y"
                          disabled={isFormDisabled}
                          maxLength={2000}
                          {...field}
                          value={field.value || ""}
                      ...
                      <div className="flex justify-between items-start mt-1">
                        <p className="text-xs text-gray-500">
                          List each allergy with its reaction. This appears at
                          the top of the patient chart for every prescriber.
                        </p>
