...
const STATUS_CONFIG: Record<string, { icon: typeof Pill; dot: string; bg: string; text: string; label: string }> = {
  ...
  rejected: { icon: XCircle, dot: "bg-red-600", bg: "bg-red-100", text: "text-red-800", label: "Rejected by Pharmacy" },
...
export function MedicationsTab({ patientId, patientName = "" }: MedicationsTabProps) {
  ...
  const fetchEmrMedications = useCallback(async () => {
    if (!user?.id) return;
    const url = `/api/basic-emr/medications?patientId=${encodeURIComponent(patientId)}`;
    const fetchOnce = async () => {
      const res = await fetch(url, { credentials: "include", cache: "no-store" });
      const json = await res.json().catch(() => ({} as { success?: boolean; data?: Medication[]; error?: string }));
      return { ok: res.ok, json } as { ok: boolean; json: { success?: boolean; data?: Medication[]; error?: string } };
    };
    ...
    if (result.ok && result.json.success && result.json.data) {
      setEmrMedications(result.json.data);
    } else {
      console.error("Error fetching EMR medications:", result.json.error);
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="text-gray-900 flex items-center gap-1.5">
                    ...
                    {rx.prescriber
                      ? `${(rx.prescriber as { prefix?: string | null }).prefix || "Dr."} ${rx.prescriber.first_name} ${rx.prescriber.last_name}`
                      : "Unknown provider"}
                  </p>
                </div>
                {rx.tracking_number && (
                  <div className="sm:col-span-2 lg:col-span-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        href={`https://parcelsapp.com/en/tracking/${rx.tracking_number}`}
                        target="_blank"
