    ...
    try {
      ...
      const { data: appointment } = await this.supabase
        ...
        .select(
          `
          provider_id,
          provider:providers(prefix, first_name, last_name)
        `,
        )
      ...
      const providerName = `${provider.prefix || "Dr."} ${provider.first_name} ${provider.last_name}`;
  ...
  async createAdHocCoachingEncounter(
    ...
    try {
      ...
      const coachingEncounterData: CreateEncounterData = {
        ...
        provider: `${(providerData as { prefix?: string | null }).prefix || "Dr."} ${providerData.first_name} ${providerData.last_name}`,
