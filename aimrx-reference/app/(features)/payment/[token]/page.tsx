...
export default function PaymentPage() {
  ...
  // Forwarded by BillPatientModal's "Charge Now" redirect so the server
  // can (a) build the right success-page URL with from=provider-dashboard
  // and (b) respect the provider's receipt-to-patient checkbox instead of
  // force-flipping it to TRUE on the patient-link path.
  const from = searchParams?.get("from") || undefined;
  // `srp` is the provider's current "send receipt to patient" checkbox
  // state, plumbed through the URL by BillPatientModal so a reused token
  // (where generate-link was skipped) still gets the up-to-date preference
  // applied server-side. Only meaningful when from === 'provider-dashboard';
  // patient-link flows ignore this and force receipt=TRUE.
  const srpParam = searchParams?.get("srp");
  const srp =
    srpParam === "1" ? true : srpParam === "0" ? false : undefined;
  ...
  const handleProceedToPayment = async () => {
    setProcessing(true);
    try {
      // Get hosted payment token from our API. Forward `from` so the server
      // builds the correct success-redirect URL (provider-dashboard vs the
      // default patient-link flow), and forward `sendReceiptToPatient` (when
      // provider-dashboard sent it via the srp query param) so a reused
      // payment_token still picks up the provider's current checkbox state.
      const response = await fetch("/api/payments/get-hosted-token", {
        ...
        body: JSON.stringify({
          paymentToken: token,
          from,
          ...(typeof srp === "boolean" ? { sendReceiptToPatient: srp } : {}),
        }),
      });
  ...
  if (paymentDetails.paymentStatus === "processing") {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <Card className="w-full max-w-md">
          <CardHeader>
          ...
          <CardContent>
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                This payment is currently being processed. Please wait a moment and refresh
                this page. If you have any questions, please contact your healthcare provider.
              </AlertDescription>
            </Alert>
          </CardContent>
        </Card>
  ...
  // Carroll incident). Payment links never expire on the patient-facing
  // page. The orange "Payment Link Expired" screen and the bottom
  // expiration notice have been removed entirely.
      ...
      <div className="container max-w-3xl mx-auto">
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {paymentDetails.prescriptions && paymentDetails.prescriptions.length > 1 ? (
                  <>
                    {paymentDetails.prescriptions.map((rx) => (
                      <div key={rx.id} className="flex justify-between items-center py-2 border-b" data-testid={`payment-item-${rx.id}`}>
                        <div>
                ...
                {parseFloat(consultationFee) > 0 && (
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-gray-700">Oversight & Monitoring Fees</span>
            ...
            {/* Expiration notice removed — payment links never expire
                (Joseph rule, May 11 2026). */}
          </CardContent>
        </Card>
