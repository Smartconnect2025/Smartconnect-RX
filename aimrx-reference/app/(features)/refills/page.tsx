...
const formatStatusLabel = (status: string) => {
  ...
  // Greenwich's "paused" really means actively-being-compounded — show that.
...
export default function RefillsPage() {
  ...
  const loadRefills = useCallback(async () => {
    ...
    if (data) {
      const doctorName = providerData
        ? `${(providerData as { prefix?: string | null }).prefix || "Dr."} ${providerData.first_name} ${providerData.last_name}`
        : "Unknown Provider";
  ...
  const loadScheduledRefills = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .gte("next_refill_date", new Date().toISOString())
          ...
          <>
            {filteredScheduled.length === 0 ? (
              <div className="bg-white border border-border rounded-lg p-12 text-center">
                ...
                <p className="text-muted-foreground mb-6">
                  Prescriptions with refills will appear here when a refill frequency is set
                </p>
              </div>
            ) : (
              <div className="bg-white border border-border rounded-lg overflow-hidden">
