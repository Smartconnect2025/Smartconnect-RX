...
import { Loader2, Search, AlertCircle, CheckCircle2, ChevronDown, ChevronRight, RefreshCw } from "lucide-react";
...
export default function DigitalRxInspectorPage() {
  ...
  async function runReconcile() {
    if (!confirm("Run a full Greenwich reconcile sweep right now? This will check every stuck order and update statuses.")) return;
    setReconciling(true);
    setReconcileResult(null);
    try {
      const r = await fetch("/api/admin/digitalrx-reconcile-now", {
        method: "POST",
        credentials: "include",
      });
      const json = await r.json();
      if (!r.ok || !json.ok) {
        setReconcileResult(`Failed: ${json.error || `HTTP ${r.status}`}`);
      } else {
        const summary = json.result
          ? `Done in ${(json.ms / 1000).toFixed(1)}s. ${JSON.stringify(json.result)}`
          : `Done in ${(json.ms / 1000).toFixed(1)}s.`;
        setReconcileResult(summary);
      }
    } catch (e) {
      setReconcileResult(`Error: ${e instanceof Error ? e.message : String(e)}`);
    } finally {
    ...
    <div className="container mx-auto p-6 max-w-5xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Look up the raw response Greenwich is sending us for any queue ID.
        ...
        <CardContent>
          ...
          >
            <Input
              ...
              autoFocus
            />
            <Button data-testid="button-check-greenwich" type="submit" disabled={loading || !queueId.trim()}>
      ...
      <Card className="border-amber-200 bg-amber-50/50">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Run Full Reconcile Now</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground mb-3">
            Triggers the same job that runs every 4 hours — sweeps every stuck Greenwich
            order, pulls the newest entry, and updates statuses. Use this when you don&apos;t
            want to wait for the next scheduled run. Safe to run anytime; if a sweep is
            already in progress this will skip cleanly.
          </p>
          <Button
          ...
          >
            ...
            {reconciling ? "Reconciling… (may take 30-90 sec)" : "Run Reconcile Sweep"}
          </Button>
          {reconcileResult && (
            <pre
              data-testid="text-reconcile-result"
              className="mt-3 text-xs font-mono bg-white border rounded p-3 max-h-64 overflow-auto whitespace-pre-wrap break-all"
            >
              {reconcileResult}
            </pre>
          )}
          ...
          {isEmptyBody && (
            <Card className="border-amber-200 bg-amber-50">
              <CardContent className="pt-6">
                <div className="font-semibold text-amber-900">Empty response from Greenwich</div>
                <p className="text-sm text-amber-800 mt-1">
                  Greenwich returned <strong>200 OK</strong> with an empty body. This means the
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">
                  Greenwich Returned {sortedEntries.length}{" "}
                    ...
                    >
                      ...
                      {isDiscardedDuplicate && (
                        <p className="text-xs text-gray-500 -mt-1 mb-3">
                          ...
                          Greenwich discarded, not a rejection of the patient&apos;s order. The
