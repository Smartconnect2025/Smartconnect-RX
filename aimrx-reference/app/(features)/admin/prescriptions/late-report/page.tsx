...
// so it should not appear in the late-orders report.
...
const isLate = (rx: RxRow): { late: boolean; hrsAtPharm: number; hrsSilent: number } => {
  if (!rx.sentToPharmacyAt) return { late: false, hrsAtPharm: 0, hrsSilent: 0 };
  if (TERMINAL_STATUSES.includes(rx.status?.toLowerCase())) return { late: false, hrsAtPharm: 0, hrsSilent: 0 };
  const hrsAtPharm = Math.round((Date.now() - new Date(rx.sentToPharmacyAt).getTime()) / 3600000);
    ...
    (async () => {
      try {
        ...
        const lateList = list
          ...
          .sort((a, b) =>
            new Date(a.sentToPharmacyAt || 0).getTime() - new Date(b.sentToPharmacyAt || 0).getTime()
          );
        setRows(lateList);
      } catch (e: any) {
  ...
  return (
    <>
      <style>{`
        ...
        .status { display: inline-block; padding: 1px 6px; border-radius: 8px; font-size: 9px; font-weight: 600; text-transform: uppercase; }
      ...
      <div className="print-page">
        <div className="header">
          <div>
            <h1>AIMRx — Late Orders Report</h1>
          ...
          <div className="meta">
            ...
            <div>app.aimrx.com / admin / prescriptions</div>
          </div>
        </div>

        <div className="summary">
          <h2><Flag style={{ width: 14, height: 14 }} /> {rows.length} late orders flagged</h2>
          <p>All orders below have been <b>paid by the customer</b> and <b>submitted to the pharmacy</b>, but have not yet shipped, been picked up, or marked delivered after more than {LATE_THRESHOLD_HOURS} hours.</p>
          <div className="rule"><b>Late rule:</b> payment cleared + sent to pharmacy &gt; {LATE_THRESHOLD_HOURS}h ago + status not in (shipped / delivered / picked up / cancelled)</div>
        </div>
        ...
        {error && (
          <div className="empty-state" style={{ color: "#DC2626" }}>
            <p><b>Failed to load:</b> {error}</p>
          </div>
        )}

        {!loading && !error && rows.length === 0 && (
          <div className="empty-state">
            <p style={{ fontSize: 14, color: "#059669" }}><b>✓ Great news — no late orders right now.</b></p>
            <p>Every paid + submitted order is within the {LATE_THRESHOLD_HOURS}-hour pharmacy turnaround window, or has already shipped.</p>
          </div>
        )}

        {!loading && !error && pharmacyList.map(([phName, phRows]) => (
          <div className="pharmacy-section" key={phName}>
            {pharmacyList.length > 1 && (
              <h3>{phName} — {phRows.length} order{phRows.length === 1 ? "" : "s"}</h3>
            )}
            <table>
              <thead>
                <tr>
                ...
                {phRows.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td className="med">{r.medication}{r.dosage && <small>{r.dosage}</small>}</td>
                      <td className="sig">{r.sig}</td>
                      <td>{r.quantity}</td>
                      <td><span className={`status ${r.status.toLowerCase()}`}>{r.status}</span></td>
                      <td className="ts">{fmtDateTimeET(r.sentToPharmacyAt)}<small>ET</small></td>
                      <td className="ts">{fmtDateTimeET(r.statusUpdatedAt)}<small>ET</small></td>
                      <td className="stuck">
        ...
        <div className="footer">
          <div>AIMRx Portal · Confidential — share only with authorized pharmacy &amp; AIM staff</div>
