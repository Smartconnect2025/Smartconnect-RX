...
export default function PrescriptionStep1Page() {
  ...
  // but preserve catalog selection if the user came from the medication catalog
  useEffect(() => {
    const preserveEncounter = !!(preselectedPatientId && encounterId);
    const hasCatalogSelection = !!sessionStorage.getItem("prescriptionFormData");
    clearPrescriptionSession({
      preserveEncounterContext: preserveEncounter,
      preserveCatalogSelection: hasCatalogSelection,
    });
                  ...
                  {patients.map((patient, index) => (
                    ...
                    >
                      ...
                      <TableCell className="px-4 sm:px-6 py-4 text-gray-900 border-none">
                        {patient.dateOfBirth
                          ? new Date(String(patient.dateOfBirth).slice(0, 10) + "T00:00:00").toLocaleDateString()
                          : "N/A"}
                      </TableCell>
                      <TableCell className="px-4 sm:px-6 py-4 text-gray-900 border-none">
                        {patient.email || "N/A"}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    {selectedPatient.dateOfBirth
                      ? new Date(
                          String(selectedPatient.dateOfBirth).slice(0, 10) + "T00:00:00",
                        ).toLocaleDateString()
                      : ""}{" "}
