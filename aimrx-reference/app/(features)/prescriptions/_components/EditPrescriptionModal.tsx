...
function buildFormData(prescription: Prescription) {
  return {
    ...
    refillFrequencyDays: prescription.refillFrequencyDays ? String(prescription.refillFrequencyDays) : "",
...
export function EditPrescriptionModal({
  ...
  const handleSave = async () => {
    ...
    if (refillCount > 0 && (!formData.refillFrequencyDays || parseInt(formData.refillFrequencyDays) <= 0)) {
      toast.error("Refill frequency is required when refills are set");
      return;
    }
    ...
    const profitCents = formData.profitFee
      ...
      : 0;
    const refillFrequencyDays = refillCount > 0 && formData.refillFrequencyDays
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-4">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  setFormData((prev) => ({
                    ...prev,
                    refills: val,
                    refillFrequencyDays: count > 0 && !prev.refillFrequencyDays ? "30" : count === 0 ? "" : prev.refillFrequencyDays,
                  }));
                }}
                min="0"
                max="12"
          ...
          {parseInt(formData.refills) > 0 && (
            <div>
              <Label htmlFor="refillFrequencyDays">Refill Frequency (days) *</Label>
              <Input
                id="refillFrequencyDays"
                type="number"
                value={formData.refillFrequencyDays}
                onChange={(e) =>
                  setFormData({ ...formData, refillFrequencyDays: e.target.value })
                }
                min="1"
                placeholder="e.g., 30"
              />
          ...
          {/* Pricing — read-only. Set from medication catalog and pharmacy profile. */}
          <div className="border-t pt-4 space-y-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Set automatically from medication pricing.
              </p>
            </div>
            <div>
