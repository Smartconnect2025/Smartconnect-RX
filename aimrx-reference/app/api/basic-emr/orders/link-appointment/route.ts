      ...
      if (orderUserPatientError && orderUserPatientError.code === "PGRST116") {
        ...
        if (appointment.provider_id) {
          ...
          if (mappingInsertError) {
            console.error("Mapping insert error (non-fatal):", mappingInsertError.message);
          } else {
            const { error: syncError } = await adminClient.rpc("sync_group_patient_mappings_for_patient", {
              p_patient_id: newPatient.id,
              p_creator_provider_id: appointment.provider_id,
            });
            if (syncError) {
              console.error("Clinic patient sync RPC error (non-fatal):", syncError.message);
            }
          }
        }
