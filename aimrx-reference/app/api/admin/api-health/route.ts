...
export async function GET() {
  ...
  try {
    ...
    const coreChecks = healthChecks.filter((c) => c.name !== "H2H DigitalRx API");
    const errorCount = coreChecks.filter((c) => c.status === "error").length;
    const degradedCount = coreChecks.filter(
      (c) => c.status === "degraded",
    ).length;
