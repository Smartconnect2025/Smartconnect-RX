...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { firstName, lastName, email, phone, password, tierLevel, npiNumber, medicalLicense, licenseState, companyName, physicalAddress, billingAddress } = body;
    ...
    if (companyName && providerData) {
      try {
        const { error: syncError } = await supabaseAdmin.rpc("sync_provider_to_group_patients", {
          p_provider_id: providerData.id,
        });
        if (syncError) {
          console.error("Company patient sync RPC error (non-fatal):", syncError.message, { providerId: providerData.id, companyName });
        }
      } catch (syncError) {
        console.error("Company patient sync error (non-fatal):", syncError);
      }
    }
            ...
            <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              ...
              <div style="background: #DBEAFE; border-left: 4px solid #2563EB; padding: 15px; margin: 20px 0; border-radius: 4px;">
                ...
                <ol style="margin: 0; padding-left: 20px; font-size: 14px; color: #1E3A8A;">
                  ...
                  <li style="margin-bottom: 8px;">Enter your NPI number, medical license details, and contact information</li>
              ...
              <div style="background: #E0F2FE; border-left: 4px solid #0284C7; padding: 15px; margin: 20px 0; border-radius: 4px;">
                <p style="margin: 0; font-size: 14px; color: #0C4A6E;">
                  <strong>📋 Account Activation:</strong> Our team will review your NPI number and credentials once your profile is complete. Your account will be fully activated upon successful verification. We will notify you by email once your account has been approved.
                </p>
              </div>
