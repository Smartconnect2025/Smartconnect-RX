...
export async function GET() {
  try {
    ...
    const { data: rawPrescriptions, error } = await query;
    ...
    const prescriptions = (rawPrescriptions || []).filter((rx) => {
      ...
      const prescriber = Array.isArray(rx.prescriber) ? rx.prescriber[0] : rx.prescriber;
      if (prescriber) {
        const fullName = (prescriber as { raw_user_meta_data?: { full_name?: string } })?.raw_user_meta_data?.full_name || "";
        if (HIDDEN_PROVIDER_LASTNAMES.some((ln) => fullName.toLowerCase().includes(ln))) return false;
      }
      return true;
    });
