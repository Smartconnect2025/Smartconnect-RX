...
export async function GET() {
  ...
  try {
    ...
    // Fetch the EFFECTIVE tier discount (only for non-pharmacy-admins).
    //
    // Resolution order (handled inside getEffectiveTierDiscountForUser):
    //   1. The caller's OWN tier (regular provider, OR an assistant whose
    //      admin/supervising provider has set a per-assistant override).
    //   2. If the caller is an active delegate AND has no own tier, the
    //      SUPERVISING provider's tier (preserves the legacy behavior).
    //   3. Otherwise 0%.
    //
    // The same helper is used at checkout in
    // app/api/prescriptions/submit/route.ts and on the wizard step 3
    // display endpoint, so the catalog price the user sees and the
    // patient_price actually charged are identical.
    let discountPercentage = 0;
    if (!isPharmacyAdmin) {
      const tierInfo = await getEffectiveTierDiscountForUser(
        supabase,
        user.id,
      );
