...
async function sendStatusEmail(prescriptionId: string, newStatus: string, trackingNumber?: string) {
  ...
  if (alreadySent) {
    console.log(`[tracking-sync] Skipping duplicate ${emailType} email for ${prescriptionId} — already sent`);
    return;
  }
  ...
  const APP_URL = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL || "https://app.aimrx.com";
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(`[tracking-sync] Status email sent: ${emailType} → ${patient.email} for ${prescriptionId}`);
    ...
    if (msg.includes("duplicate") || msg.includes("already exists") || msg.includes("in-flight")) {
      ...
      try {
        ...
        if (existing?.id) {
          ...
          console.log(`[easypost] Found existing tracker: ${existing.id} for ${trackingNumber}`);
...
export async function applyTrackingUpdate(
  ...
  console.log(
    `[tracking-sync] Updated prescription ${prescriptionId} via ${source}: ${update.fedexStatus}${newStatus ? ` (status → ${newStatus})` : ""}`,
  );
  ...
  if (newStatus && newStatus !== rx.status) {
    ...
    sendStatusEmail(prescriptionId, newStatus, trackingNum || undefined).catch((err) =>
      console.error(`[tracking-sync] Email dispatch error for ${prescriptionId}:`, err),
    );
  }
