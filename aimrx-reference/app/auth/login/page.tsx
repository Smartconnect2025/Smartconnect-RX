...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Trusted-device gate: if this browser carries a valid trust cookie,
      // the server will finalize the session (matching the post-2FA path)
      // and tell us where to land. Any non-ok outcome falls through to the
      // existing 2FA flow below — never a silent skip.
      try {
        const trustRes = await fetch("/api/auth/mfa/trust-check", {
          method: "POST",
        ...
        if (trustRes.ok) {
          const trustData = await trustRes.json().catch(() => null);
          if (trustData?.trusted && typeof trustData.redirect === "string") {
            window.location.href = trustData.redirect;
            return;
          }
        }
      } catch {
        // Network/parse failure — fall through to 2FA. Default-deny.
      }
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#1E3A8A] via-[#2563EB] to-[#00AEEF] overflow-hidden flex flex-col relative">
        ...
        <div className="pt-2 pb-2 text-center z-10">
          <div className="flex flex-col items-center gap-2">
            <div className="bg-white rounded-2xl px-5 py-3 shadow-xl inline-flex items-center justify-center">
              <img
                src="/logo-header.png"
                alt="AIM Logo"
