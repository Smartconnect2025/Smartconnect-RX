...
export function ProviderHeader() {
  ...
  // Load provider name and company name. Works for both providers (looked up
  // directly in `providers`) and delegates (looked up via the active
  // delegation that links them to their authorizing provider).
  useEffect(() => {
    const loadProviderName = async () => {
      ...
      const { data: providerRow } = await supabase
        .from("providers")
        .select("prefix, first_name, last_name, company_name")
        .eq("user_id", user.id)
        .maybeSingle();

      if (providerRow) {
        const px = (providerRow.prefix as string | null) || "Dr.";
        setProviderName(`${px} ${providerRow.first_name} ${providerRow.last_name}`);
        if (providerRow.company_name) {
          setCompanyName(providerRow.company_name);
        }
        return;
      }

      // No matching provider row — user is likely a delegate. Look up the
      // authorizing provider through the active delegation.
      const { data: delegationRow } = await supabase
        .from("delegations")
        .select(
          "providers!inner(prefix, first_name, last_name, company_name)",
        )
      ...
      if (provider) {
        const px = provider.prefix || "Dr.";
        setProviderName(
          `${px} ${provider.first_name} ${provider.last_name} (Assistant)`,
        );
